...
@section('content')
    <div class="container-fluid flex-grow-1 container-p-y">
        ...
        <div class="card ">
            <div class="card-header d-flex justify-content-between align-items-center border-bottom-1">
                <h5 class="mb-0">Expense {{ $categorysettings->expense_category == 2 ? 'Sub' : '' }} Category</h5>
                <button type="button" class="btn btn-primary {{ Auth::user()->access->expense == 1 ? 'disabled' : '' }}" data-bs-toggle="modal"
                    data-bs-target="#addmodals">Add Expense {{ $categorysettings->expense_category == 2 ? 'Sub' : '' }} Category</button>
            </div>
                    ...
                    <table class="table" id="myTable">
                        ...
                        <tbody class="table-border-bottom-0">
                            ...
                            <tr>
                                ...
                                <td>{{ $expensesubcategory->expenseCategory->name ?? 'Expense Category Not Assigned' }}</td>
                                <td>{{ $expensesubcategory->name }}</td>
                                <td>{{ $expensesubcategory->description ?? 'N/A' }}</td>
                                <td>
                                    @if($expensesubcategory->status == '1')
                                ...
                                <td>
                                    <div class="d-flex align-items-center gap-1 cursor-pointer">
                                            ...
                                            data-bs-target="#editModal{{ $expensesubcategory->id }}"><i
                                                    class="bx bx-edit-alt me-1"></i> Edit</a>
                                                     @if($expensesubcategory->id != 14 && $expensesubcategory->id != 15)
                                        <form action="{{ route('expensesubcategory.destroy', $expensesubcategory->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-outline-danger delete-confirm {{ Auth::user()->access->expense == 1 ? 'disabled' : '' }}" ><i
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Add Expense {{ $categorysettings->expense_category == 2 ? 'Sub' : '' }} Category</h5>
    ...
    @if($expenseSubCategories->isNotEmpty())
        ...
        <div class="modal fade" id="editModal{{ $expensesubcategory->id }}">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Edit Expense {{ $categorysettings->expense_category == 2 ? 'Sub' : '' }} Category</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <form id="editIncomeCategoryForms{{ $expensesubcategory->id }}" action="{{ route('expensesubcategory.update', $expensesubcategory->id) }}">
                        @csrf
                        @method('PUT')
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                <label for="name" class="form-label">Name</label>
                                <input type="text" class="form-control name-input" id="name" name="name" value="{{ $expensesubcategory->name }}" required>
                            ...
                            <div class="mb-3">
                                <label for="expense_category_id" class="form-label">Category</label>
                                <select class="form-select" id="income_category_id" name="expense_category_id" required>
                                    <option value="">Select Category</option>
                                    @foreach ($expenseCategories as $category)
                                        <option value="{{ $category->id }}" {{ $expensesubcategory->expense_category_id == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            @endif
                            <div class="mb-3">
                                <label for="Description" class="form-label">Description</label>
                                <textarea class="form-control" id="Description" name="description" rows="3">{{ $expensesubcategory->description }}</textarea>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="status" name="status" required>
                                    ...
                                    <option value="1" {{ $expensesubcategory->status == '1' ? 'selected' : '' }}>Active</option>
                                    <option value="2" {{ $expensesubcategory->status == '2' ? 'selected' : '' }}>Inactive</option>
                                </select>
                            </div>
                        </div>
                        <div class="modal-footer">
...
<script>
    ...
    @foreach ($expenseSubCategories as $expensesubcategory)
        attachSlugListener('editModal{{ $expensesubcategory->id }}');
    @endforeach
</script>
...
<script>
    $(document).ready(function () {
        ...
        $('form#addIncomeCategoryForms button[type="submit"]').on('click', function (e) {
            ...
            $.ajax({
                url: "{{ route('expensesubcategory.store') }}",
                method: "POST",
                data: formData,
                headers: {
