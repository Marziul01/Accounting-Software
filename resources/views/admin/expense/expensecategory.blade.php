...
@section('content')
        ...
        <div class="card ">
            <div class="card-header d-flex justify-content-between align-items-center border-bottom-1">
                ...
                <button type="button" class="btn btn-primary {{ Auth::user()->access->expense == 1 ? 'disabled' : '' }}" data-bs-toggle="modal"
                    data-bs-target="#addmodals">Add Expense Category</button>
            </div>
                    ...
                    <table class="table" id="myTable">
                        ...
                        <tbody class="table-border-bottom-0">
                            ...
                            <tr>
                                ...
                                <td>{{ $expenseCategory->description ?? 'N/A' }}</td>
                                <td>
                                ...
                                <td>
                                    <div class="d-flex align-items-center gap-1 cursor-pointer">
                                            ...
                                            data-bs-target="#editModal{{ $expenseCategory->id }}"><i
                                                    class="bx bx-edit-alt me-1"></i> Edit</a>
                                                @if($expenseCategory->id != 7)
                                        <form action="{{ route('expensecategory.destroy', $expenseCategory->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
    ...
    @if($expenseCategories->isNotEmpty())
        ...
        <div class="modal fade" id="editModal{{ $expenseCategory->id }}">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <form id="editIncomeCategoryForms{{ $expenseCategory->id }}" action="{{ route('expensecategory.update', $expenseCategory->id) }}">
                        @csrf
                        @method('PUT')
                        <div class="modal-body">
                            <div class="mb-3">
                                <label for="name" class="form-label">Name</label>
                                <input type="text" class="form-control name-input" id="name" name="name" value="{{ $expenseCategory->name }}" required>
                               
                            </div>
                            <div class="mb-3 d-none">
                                <label for="slug" class="form-label">Slug</label>
                                <input type="text" class="form-control slug-output" id="slug" name="slug" value="{{ $expenseCategory->slug }}" readonly>
                            </div>
                            <div class="mb-3">
                                <label for="Description" class="form-label">Description</label>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="status" name="status" required>
                                    ...
                                    <option value="2" {{ $expenseCategory->status == '2' ? 'selected' : '' }}>Inactive</option>
                                </select>
                            </div>
                        </div>
                        <div class="modal-footer">
...
<script>
    $(document).ready(function () {
        ...
        $('form#addIncomeCategoryForms button[type="submit"]').on('click', function (e) {
            ...
            $.ajax({
                url: "{{ route('expensecategory.store') }}",
                method: "POST",
                data: formData,
                headers: {
