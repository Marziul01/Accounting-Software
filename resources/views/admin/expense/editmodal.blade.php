<div class="modal fade" id="editExpenseModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <form id="editExpenseForm" method="POST" action="{{ route('expense.update', $expense->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control name-input" id="editName" name="name"
                                required value="{{ $expense->name }}">
                        </div>
                        <div class="mb-3 d-none">
                            <label for="slug" class="form-label">Slug</label>
                            <input type="text" class="form-control slug-output" id="editslug" name="slug"
                                readonly value="{{ $expense->slug }}">
                        </div>
                        <div class="mb-3">
                            <label for="editCategory" class="form-label">Category</label>
                            <select class="form-select category-select" id="editCategory1" name="expense_category_id" required>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select subcategory-select" id="editSubCategory"
                                name="expense_sub_category_id" required data-selected="{{ $expense->expense_sub_category_id }}">
                                <option value="">Select Sub Category</option>
                            </select>
                        </div>
                        <div class="mb-3">
                            <label for="editAmount" class="form-label">Amount</label>
                            <input type="number" class="form-control" id="editAmount" name="amount" required value="{{ $expense->amount }}">
                        </div>
                        <div class="mb-3">
                            <label for="editDate" class="form-label">Expense Date</label>
                            <input type="date" class="form-control myDate" id="editDate" name="date" required value="{{ $expense->date }}">
                        </div>
                        <div class="mb-3">
                            <label for="editDescription" class="form-label">Description</label>
                            <textarea class="form-control" id="editDescription" name="description" rows="3">{{ $expense->description }}</textarea>
                        </div>
                        <div class="mb-3">
