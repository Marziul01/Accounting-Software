...
@section('content')
                ...
                <div class="table-responsive">
                    <table class="table" id="myTable">
                        ...
                        <tbody class="table-border-bottom-0">
                            ...
                            @foreach ($liabilityCategories as $liabilityCategory )
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $liabilityCategory->name }}</td>
                                <td>{{ $liabilityCategory->description ?? 'N/A' }}</td>
                                <td>
                                    @if($liabilityCategory->status == '1')
                                ...
                                <td>
                                    <div class="d-flex align-items-center gap-1 cursor-pointer">
                                        ...
                                        @if($liabilityCategory->id != 3 && $liabilityCategory->id != 4)
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Add Liability Category</h5>
    ...
    @if($liabilityCategories->isNotEmpty())
        ...
        <div class="modal fade" id="editModal{{ $liabilityCategory->id }}">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Edit Liability Category</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <form id="editIncomeCategoryForms{{ $liabilityCategory->id }}" action="{{ route('liabilitycategory.update', $liabilityCategory->id) }}">
                        @csrf
                        @method('PUT')
                        <div class="modal-body">
                            <div class="mb-3">
                                <label for="name" class="form-label">Name</label>
                                <input type="text" class="form-control name-input" id="name" name="name" value="{{ $liabilityCategory->name }}" required>
                               
                            </div>
                            <div class="mb-3 d-none">
                                <label for="slug" class="form-label">Slug</label>
                                <input type="text" class="form-control slug-output" id="slug" name="slug" value="{{ $liabilityCategory->slug }}" readonly>
                            </div>
                            <div class="mb-3">
                                <label for="Description" class="form-label">Description</label>
                                <textarea class="form-control" id="Description" name="description" rows="3">{{ $liabilityCategory->description }}</textarea>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="status" name="status" required>
                                    ...
                                    <option value="1" {{ $liabilityCategory->status == '1' ? 'selected' : '' }}>Active</option>
                                    <option value="2" {{ $liabilityCategory->status == '2' ? 'selected' : '' }}>Inactive</option>
                                </select>
                            </div>
                        </div>
                        <div class="modal-footer">
...
<script>
    ...
    @foreach ($liabilityCategories as $liabilityCategory)
        attachSlugListener('editModal{{ $liabilityCategory->id }}');
    @endforeach
</script>
...
<script>
    $(document).ready(function () {
        ...
        $('form#addIncomeCategoryForms button[type="submit"]').on('click', function (e) {
            ...
            $.ajax({
                url: "{{ route('liabilitycategory.store') }}",
                method: "POST",
                data: formData,
                headers: {
