...
@section('content')
                ...
                <div class="table-responsive">
                    <table class="table" id="myTable">
                        ...
                        <tbody class="table-border-bottom-0">
                            @if($liabilitySubCategories->isNotEmpty())
                            @foreach ($liabilitySubCategories as $liabilitysubcategory )
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $liabilitysubcategory->liabilityCategory->name ?? 'Liability Category Not Assigned' }}</td>
                                <td>{{ $liabilitysubcategory->name }}</td>
                                <td>{{ $liabilitysubcategory->description ?? 'N/A' }}</td>
                                <td>
                                    @if($liabilitysubcategory->status == '1')
                                ...
                                <td>
                                    <div class="d-flex align-items-center gap-1 cursor-pointer">
                                            ...
                                            data-bs-target="#editModal{{ $liabilitysubcategory->id }}"><i
                                                    class="bx bx-edit-alt me-1"></i> Edit</a>
                                        <form action="{{ route('liabilitysubcategory.destroy', $liabilitysubcategory->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
            ...
            <div class="modal-content">
                ...
                <form id="addIncomeCategoryForms">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label for="liability_category_id" class="form-label">Category</label>
                            <select class="form-select" id="liability_category_id" name="liability_category_id" required>
    ...
    @if($liabilitySubCategories->isNotEmpty())
        ...
        <div class="modal fade" id="editModal{{ $liabilitysubcategory->id }}">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <form id="editIncomeCategoryForms{{ $liabilitysubcategory->id }}" action="{{ route('liabilitysubcategory.update', $liabilitysubcategory->id) }}">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                <label for="liability_category_id" class="form-label">Category</label>
                                <select class="form-select" id="income_category_id" name="liability_category_id" required>
                                    <option value="">Select Category</option>
                                    @foreach ($liabilityCategories as $category)
                                        <option value="{{ $category->id }}" {{ $liabilitysubcategory->liability_category_id == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="mb-3">
                                <label for="Description" class="form-label">Description</label>
                                <textarea class="form-control" id="Description" name="description" rows="3">{{ $liabilitysubcategory->description }}</textarea>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="status" name="status" required>
                                    ...
                                    <option value="1" {{ $liabilitysubcategory->status == '1' ? 'selected' : '' }}>Active</option>
                                    <option value="2" {{ $liabilitysubcategory->status == '2' ? 'selected' : '' }}>Inactive</option>
                                </select>
                            </div>
                        </div>
                        <div class="modal-footer">
...
<script>
    ...
    @foreach ($liabilitySubCategories as $liabilitysubcategory)
        attachSlugListener('editModal{{ $liabilitysubcategory->id }}');
    @endforeach
</script>
...
<script>
    $(document).ready(function () {
        ...
        $('form#addIncomeCategoryForms button[type="submit"]').on('click', function (e) {
            ...
            $.ajax({
                url: "{{ route('liabilitysubcategory.store') }}",
                method: "POST",
                data: formData,
                headers: {
