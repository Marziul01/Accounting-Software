...
<head>
    ...
    <title>সম্পূর্ণ দায় রিপোর্ট</title>
...
<div class="container-fluid my-4">
    ...
    @foreach($categories as $category)
        ...
        @if($category->liabilitySubCategories->count())
        <div class="mb-5">
            ...
            @foreach($category->liabilitySubCategories as $subcategory)
                ...
                @if($subcategory->liabilitySubSubCategories->count())
                    @foreach($subcategory->liabilitySubSubCategories as $subsubcategory)
                                    ...
                                    <table class="table table-bordered m-0">
                                        <thead class="table-light">
                                                ...
                                                @foreach($liabilities->sortBy('entry_date') as $liability)
                                                        @php
                                                        $totalAddition = $liability->transactions
                                                                ->where('transaction_type', 'Deposit')
                                                                ->sum('amount');

                                                        $totalPayment = $liability->transactions
                                                                ->where('transaction_type', 'Withdraw')
                                                                ->sum('amount');

                                                        $initialLiability = $liability->amount
                                                                - $totalAddition + $totalPayment;
                                                        @endphp
                                                        <tr>
                                                        ...
                                                        <td class="text-end tiro">{{ bn_number(number_format($totalAddition, 2)) }} টাকা</td>
                        ...
                        @else
                        <div class="">
                            <strong>{{ $subcategory->name }} → {{ $subsubcategory->name }}</strong> এর জন্য কোনো দায় নেই।
                        @endif
                    @endforeach
        ...
        <table class="table table-bordered w-auto summary-box mb-0" style="min-width: 350px;">
            ...
            <tbody>
                ...
                <tr class="grand-total">
                    <td><strong>সর্বমোট দায়</strong></td>
