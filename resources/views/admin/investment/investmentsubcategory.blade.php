...
@section('content')
                ...
                <div class="table-responsive">
                    <table class="table" id="myTable">
                        ...
                        <tbody class="table-border-bottom-0">
                            @if($investmentSubCategories->isNotEmpty())
                            @foreach ($investmentSubCategories as $investmentsubcategory )
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $investmentsubcategory->investmentCategory->name ?? 'investment Category Not Assigned' }}</td>
                                <td>{{ $investmentsubcategory->name }}</td>
                                <td>{{ $investmentsubcategory->description ?? 'N/A' }}</td>
                                <td>
                                    @if($investmentsubcategory->status == '1')
                                ...
                                <td>
                                    <div class="d-flex align-items-center gap-1 cursor-pointer">
                                            ...
                                            data-bs-target="#editModal{{ $investmentsubcategory->id }}"><i
                                                    class="bx bx-edit-alt me-1"></i> Edit</a>
                                        <form action="{{ route('investmentsubcategory.destroy', $investmentsubcategory->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                            ...
                            @else
                            <tr>
                                <td colspan="4" class="text-center">No investment categories found.</td>
                            </tr>
                            @endif
                        </tbody>
                    </table>
            ...
            <div class="modal-content">
                ...
                <form id="addIncomeCategoryForms">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label for="investment_category_id" class="form-label">Category</label>
                            <select class="form-select" id="investment_category_id" name="investment_category_id" required>
    ...
    @if($investmentSubCategories->isNotEmpty())
        ...
        <div class="modal fade" id="editModal{{ $investmentsubcategory->id }}">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <form id="editIncomeCategoryForms{{ $investmentsubcategory->id }}" action="{{ route('investmentsubcategory.update', $investmentsubcategory->id) }}">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                <label for="investment_category_id" class="form-label">Category</label>
                                <select class="form-select" id="income_category_id" name="investment_category_id" required>
                                    <option value="">Select Category</option>
                                    @foreach ($investmentCategories as $category)
                                        <option value="{{ $category->id }}" {{ $investmentsubcategory->investment_category_id == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="mb-3">
                                <label for="Description" class="form-label">Description</label>
                                <textarea class="form-control" id="Description" name="description" rows="3">{{ $investmentsubcategory->description }}</textarea>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="status" name="status" required>
                                    ...
                                    <option value="1" {{ $investmentsubcategory->status == '1' ? 'selected' : '' }}>Active</option>
                                    <option value="2" {{ $investmentsubcategory->status == '2' ? 'selected' : '' }}>Inactive</option>
                                </select>
                            </div>
                        </div>
                        <div class="modal-footer">
...
<script>
    ...
    @foreach ($investmentSubCategories as $investmentsubcategory)
        attachSlugListener('editModal{{ $investmentsubcategory->id }}');
    @endforeach
</script>
...
<script>
    $(document).ready(function () {
        ...
        $('form#addIncomeCategoryForms button[type="submit"]').on('click', function (e) {
            ...
            $.ajax({
                url: "{{ route('investmentsubcategory.store') }}",
                method: "POST",
                data: formData,
                headers: {
