...
@section('content')
                    ...
                    <table class="table" id="myTable">
                        ...
                        <tbody class="table-border-bottom-0">
                            ...
                            <tr>
                                ...
                                <td>{{ $investmentCategory->name }}</td>
                                <td>{{ $investmentCategory->description ?? 'N/A' }}</td>
                                <td>
                                    @if($investmentCategory->status == '1')
                                ...
                                <td>
                                    <div class="d-flex align-items-center gap-1 cursor-pointer">
                                            ...
                                            data-bs-target="#editModal{{ $investmentCategory->id }}"><i
                                                    class="bx bx-edit-alt me-1"></i> Edit</a>
                                                    @if( $investmentCategory->id != 4 && $investmentCategory->id != 5 )
                                        <form action="{{ route('investmentcategory.destroy', $investmentCategory->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Add Investment Category</h5>
    ...
    @if($investmentCategories->isNotEmpty())
        ...
        <div class="modal fade" id="editModal{{ $investmentCategory->id }}">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Edit Investment Category</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <form id="editIncomeCategoryForms{{ $investmentCategory->id }}" action="{{ route('investmentcategory.update', $investmentCategory->id) }}">
                        @csrf
                        @method('PUT')
                        <div class="modal-body">
                            <div class="mb-3">
                                <label for="name" class="form-label">Name</label>
                                <input type="text" class="form-control name-input" id="name" name="name" value="{{ $investmentCategory->name }}" required>
                               
                            </div>
                            <div class="mb-3 d-none">
                                <label for="slug" class="form-label">Slug</label>
                                <input type="text" class="form-control slug-output" id="slug" name="slug" value="{{ $investmentCategory->slug }}" readonly>
                            </div>
                            <div class="mb-3">
                                <label for="Description" class="form-label">Description</label>
                                <textarea class="form-control" id="Description" name="description" rows="3">{{ $investmentCategory->description }}</textarea>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="status" name="status" required>
                                    ...
                                    <option value="1" {{ $investmentCategory->status == '1' ? 'selected' : '' }}>Active</option>
                                    <option value="2" {{ $investmentCategory->status == '2' ? 'selected' : '' }}>Inactive</option>
                                </select>
                            </div>
                        </div>
                        <div class="modal-footer">
...
<script>
    $(document).ready(function () {
        ...
        $('form#addIncomeCategoryForms button[type="submit"]').on('click', function (e) {
            ...
            $.ajax({
                url: "{{ route('investmentcategory.store') }}",
                method: "POST",
                data: formData,
                headers: {
