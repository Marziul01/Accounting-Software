...
@section('content')
        ...
        <div class="card ">
            <div class="card-header d-flex justify-content-between align-items-center border-bottom-1">
                ...
                <button type="button" class="btn btn-primary {{ Auth::user()->access->asset == 1 ? 'disabled' : '' }}" data-bs-toggle="modal"
                    data-bs-target="#addmodals">Add Asset Category</button>
            </div>
                    ...
                    <table class="table" id="myTable">
                        ...
                        <tbody class="table-border-bottom-0">
                            ...
                            <tr>
                                ...
                                <td>{{ $assetCategory->description ?? 'N/A' }}</td>
                                <td>
                                    @if($assetCategory->status == '1')
                                ...
                                <td>
                                    <div class="d-flex align-items-center gap-1 cursor-pointer">
                                            ...
                                            data-bs-target="#editModal{{ $assetCategory->id }}"><i
                                                    class="bx bx-edit-alt me-1"></i> Edit</a>
                                        @if($assetCategory->id != 4 && $assetCategory->id != 5)
                                        <form action="{{ route('assetcategory.destroy', $assetCategory->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
    ...
    @if($assetCategories->isNotEmpty())
        ...
        <div class="modal fade" id="editModal{{ $assetCategory->id }}">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Edit Asset Category</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <form id="editIncomeCategoryForms{{ $assetCategory->id }}" action="{{ route('assetcategory.update', $assetCategory->id) }}">
                        @csrf
                        @method('PUT')
                        <div class="modal-body">
                            <div class="mb-3">
                                <label for="name" class="form-label">Name</label>
                                <input type="text" class="form-control name-input" id="name" name="name" value="{{ $assetCategory->name }}" required>
                               
                            </div>
                            <div class="mb-3 d-none">
                                <label for="slug" class="form-label">Slug</label>
                                <input type="text" class="form-control slug-output" id="slug" name="slug" value="{{ $assetCategory->slug }}" readonly>
                            </div>
                            <div class="mb-3">
                                <label for="Description" class="form-label">Description</label>
                                <textarea class="form-control" id="Description" name="description" rows="3">{{ $assetCategory->description }}</textarea>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="status" name="status" required>
                                    ...
                                    <option value="1" {{ $assetCategory->status == '1' ? 'selected' : '' }}>Active</option>
                                    <option value="2" {{ $assetCategory->status == '2' ? 'selected' : '' }}>Inactive</option>
                                </select>
                            </div>
                        </div>
                        <div class="modal-footer">
...
<script>
    ...
    @foreach ($assetCategories as $assetCategory)
        attachSlugListener('editModal{{ $assetCategory->id }}');
    @endforeach
</script>
