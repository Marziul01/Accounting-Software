...
@section('content')
                ...
                <div class="table-responsive">
                    <table class="table" id="myTable">
                        ...
                        <tbody class="table-border-bottom-0">
                            @if($assetSubCategories->isNotEmpty())
                            @foreach ($assetSubCategories as $assetsubcategory )
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $assetsubcategory->assetCategory->name ?? 'Asset Category Not Assigned' }}</td>
                                <td>{{ $assetsubcategory->name }}</td>
                                <td>{{ $assetsubcategory->description ?? 'N/A' }}</td>
                                <td>
                                    @if($assetsubcategory->status == '1')
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Add Asset Sub Category</h5>
    ...
    @if($assetSubCategories->isNotEmpty())
        ...
        <div class="modal fade" id="editModal{{ $assetsubcategory->id }}">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Edit Asset Sub Category</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <form id="editIncomeCategoryForms{{ $assetsubcategory->id }}" action="{{ route('assetsubcategory.update', $assetsubcategory->id) }}">
                        @csrf
                        @method('PUT')
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                <label for="name" class="form-label">Name</label>
                                <input type="text" class="form-control name-input" id="name" name="name" value="{{ $assetsubcategory->name }}" required>
                               
                            </div>
                            <div class="mb-3">
                                <label for="slug" class="form-label">Slug</label>
                                <input type="text" class="form-control slug-output" id="slug" name="slug" value="{{ $assetsubcategory->slug }}" readonly>
                            </div>
                            <div class="mb-3">
                                <label for="asset_category_id" class="form-label">Category</label>
                                <select class="form-select" id="income_category_id" name="asset_category_id" required>
                                    <option value="">Select Category</option>
                                    @foreach ($assetCategories as $category)
                                        <option value="{{ $category->id }}" {{ $assetsubcategory->asset_category_id == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="mb-3">
                                <label for="Description" class="form-label">Description</label>
                                <textarea class="form-control" id="Description" name="description" rows="3">{{ $assetsubcategory->description }}</textarea>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="status" name="status" required>
                                    ...
                                    <option value="1" {{ $assetsubcategory->status == '1' ? 'selected' : '' }}>Active</option>
                                    <option value="2" {{ $assetsubcategory->status == '2' ? 'selected' : '' }}>Inactive</option>
                                </select>
                            </div>
                        </div>
                        <div class="modal-footer">
...
<script>
    ...
    @foreach ($assetSubCategories as $assetsubcategory)
        attachSlugListener('editModal{{ $assetsubcategory->id }}');
    @endforeach
</script>
...
<script>
    $(document).ready(function () {
        ...
        $('form#addIncomeCategoryForms button[type="submit"]').on('click', function (e) {
            ...
            $.ajax({
                url: "{{ route('assetsubcategory.store') }}",
                method: "POST",
                data: formData,
                headers: {
