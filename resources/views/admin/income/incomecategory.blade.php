        ...
        <div class="card ">
            <div class="card-header d-flex justify-content-between align-items-center border-bottom-1">
                ...
                <button type="button" class="btn btn-primary {{ Auth::user()->access->income == 1 ? 'disabled' : '' }}" data-bs-toggle="modal"
                    data-bs-target="#addmodals">Add Income Category</button>
            </div>
                                ...
                                <td>
                                    <div class="d-flex align-items-center gap-1 cursor-pointer">
                                            ...
                                            data-bs-target="#editModal{{ $incomecategory->id }}"><i
                                                    ...
                                                    @if($incomecategory->id != 13)
                                        <form action="{{ route('incomecategory.destroy', $incomecategory->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
    ...
    @if($incomeCategories->isNotEmpty())
        ...
        <div class="modal fade" id="editModal{{ $incomecategory->id }}">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Edit Income Category</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <form id="editIncomeCategoryForms{{ $incomecategory->id }}" action="{{ route('incomecategory.update', $incomecategory->id) }}">
                        @csrf
                        @method('PUT')
                        <div class="modal-body">
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                <label for="Description" class="form-label">Description</label>
                                <textarea class="form-control" id="Description" name="desc" rows="3">{{ $incomecategory->desc }}</textarea>
...
<script>
    $(document).ready(function () {
        ...
        $('form#addIncomeCategoryForms button[type="submit"]').on('click', function (e) {
            ...
            $.ajax({
                url: "{{ route('incomecategory.store') }}",
                method: "POST",
                data: formData,
                headers: {
